<template>
  <div class="upload-page">
    ...
    <!-- 1. 初始状态：文件拖拽上传区 -->
    <div v-if="!isDialogVisible" class="upload-trigger-area">
    ...
    >
      ...
      <template #header>
        <div class="dialog-header">
          <div class="header-left-title">{{ form.title || '上传视频' }}</div>
          ...
          <div class="header-actions">
            <span v-if="uploadPercentage < 100" class="upload-status">正在上传 {{ Math.round(uploadPercentage) }}% ...</span>
            <span v-else class="upload-status success">上传完成</span>
            <el-icon class="close-btn" @click="handleClose"><Close /></el-icon>
          </div>
        </div>
        <el-progress v-if="uploadPercentage < 100" :percentage="uploadPercentage" :show-text="false" :stroke-width="2" class="top-progress" />
      </template>
        ...
        <div class="body-content">
          ...
          <div class="left-form-section">
            ...
            <div v-show="currentStep === 0" class="step-content">
              ...
              <div class="form-group mt-4">
                ...
                <div class="thumbnail-selector">
                  ...
                  <!-- 自动生成的截图 -->
                  <div 
                    v-for="(img, idx) in autoCovers" 
                    :key="idx" 
                    class="thumb-box auto-box"
                    :class="{ selected: form.cover_url === img && !isCustomCoverSelected }"
                    @click="selectAutoCover(img)"
                  >
                    <img :src="img" class="thumb-img" />
                  </div>
                  
                  <!-- 骨架屏 -->
                  <div v-if="autoCovers.length === 0" class="thumb-box skeleton" v-for="i in 3">
            ...
            <div v-show="currentStep === 2" class="step-content">
              ...
              <p style="color:#606060; font-size:14px; margin-bottom: 20px;">
                我们会检查你的视频是否存在版权问题。
              </p>
              <div class="check-result">
                <div class="check-row">
                  <span class="check-label">版权</span>
                  <span class="check-val success"><el-icon><Check /></el-icon> 未发现问题</span>
                </div>
              </div>
            </div>
            ...
            <div class="preview-card">
              ...
              <div class="preview-info">
                <div class="link-row">
                  ...
                  <div class="link-val" v-if="videoId">http://localhost/video/{{ videoId }}</div>
                  <div class="link-val" v-else>生成中...</div>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <div class="footer-right">
            ...
            <el-button type="primary" @click="submitPublish" v-if="currentStep === 3" :loading="isPublishing">发布</el-button>
          </div>
        </div>
      </template>
...
import { uploadVideoFile, publishVideo } from '../api/video';
import { uploadBanner } from '../api/user'; // 复用上传图片接口
...
const categories = [
  "宠物和动物", "电影和动画", "公益和社会活动", "教育", "科学和技术", 
  "旅游和活动", "汽车和其他交通工具", "人物和博客", "体育", "喜剧", 
  "新闻和政治", "音乐", "游戏", "娱乐", "DIY和生活百科"
];
...
const handleFileSelected = async (e) => {
  ...
  let progressTimer = setInterval(() => {
    if (uploadPercentage.value < 90) uploadPercentage.value += 10;
  }, 200);
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      if (autoCovers.value.length > 0) {
        form.cover_url = autoCovers.value[0];
      }
    }
  } catch (error) {
    clearInterval(progressTimer);
...
// 2. 封面逻辑
const selectAutoCover = (url) => {
  form.cover_url = url;
  isCustomCoverSelected.value = false;
};
const triggerCoverSelect = () => coverInput.value.click();
const handleCoverSelected = (e) => {
  const file = e.target.files[0];
  if (file) cropperRef.value.open(file);
  e.target.value = '';
};
const doUploadCustomCover = async (blob, done) => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      customCoverUrl.value = res.data.url;
      form.cover_url = res.data.url;
      isCustomCoverSelected.value = true;
      done();
    }
  } catch(e) { ElMessage.error('图片处理失败'); }
};
...
const submitPublish = async () => {
  if (!videoId.value) return ElMessage.warning('视频还未上传完成');
...
const handleClose = () => {
  ElMessageBox.confirm('上传尚未完成，确定要取消吗？视频将保存为草稿。', '提示')
    .then(() => { isDialogVisible.value = false; })
    .catch(() => {});
};
</script>
